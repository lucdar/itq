...
pub fn AddQueuePage() -> impl IntoView {
    ...
    view! {
        ...
        >
            { move || value.get().map(|result| {
                match result {
                    Ok(queue_info) => { view! {
                        <p>"Queue added: "
                            <a href={format!("/queue/{}", queue_info.url_name)}>{queue_info.display_name}</a>
                        </p>
                    }}.into_any(),
                    Err(e) => view! { <p>"Error adding queue: " {e.to_string()}</p> }.into_any(),
                }
            })}
        </Show>
    }
...
#[server]
pub async fn add_queue(display_name: String, url_name: String) -> Result<QueueInfo, ServerFnError> {
